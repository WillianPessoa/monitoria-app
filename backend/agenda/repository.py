...
def cancel_agendamento_db(agendamento_id):
    ...
    try:
        ...
        cursor.execute(
            """
            UPDATE disponibilidades
            SET status = 'DISPONIVEL', atualizado_em = CURRENT_TIMESTAMP
...
def block_slot(disponibilidade_id, monitor_id):
    ...
    try:
        cursor.execute(
            """
            SELECT status FROM disponibilidades
            WHERE id = %s AND monitor_id = %s
            FOR UPDATE
            """,
        ...
        cursor.execute(
            """
            UPDATE disponibilidades
            SET status = 'BLOQUEADO', atualizado_em = CURRENT_TIMESTAMP
            WHERE id = %s
            """,
            (disponibilidade_id,),
        )
